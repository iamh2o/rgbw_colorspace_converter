...
#                self.grid._model.set(addr, xyhsv[2])
    ...
    def next_frame(self):
        ...
        pix_arr = [[None for i in range(88)] for j in range(pix_rows)] #This thing is not recognizing pix_col in the first range method...?!?!?
        ...
        while True:
            (x1, x2, x3, x4) = (random.randint(0,87), random.randint(0,87),random.randint(0,87), random.randint(0,87))
            (hsv1, hsv2, hsv3, hsv4) = (choose_random_hsv(), choose_random_hsv(),choose_random_hsv(),choose_random_hsv())
